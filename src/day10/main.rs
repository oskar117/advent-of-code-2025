...
fn process_joltage(target_joltage: &Vec<i32>, buttons: &Vec<Vec<usize>>) -> i32 {
    ...
    let mut queue = VecDeque::from(reduce_possible_presses(&mut cache, &target_joltage, &vec![0; target_joltage.len()], 0, &buttons));
    while queue.len() > 0 {
        let queue_message = queue.pop_front().unwrap();
        ...
        new_entries.iter()
            ...
            .for_each(|element| queue.push_back(element.clone()));
    }
    0
}

fn reduce_possible_presses<'a>(cache: &mut HashSet<(Vec<i32>, &'a Vec<usize>, i32)>, target_joltage: &Vec<i32>, current_joltage: &Vec<i32>, current_press: i32, buttons: &'a Vec<Vec<usize>>) -> Vec<(Vec<i32>, &'a Vec<usize>, i32)> {
    ...
    let mut queue = Vec::<(Vec<i32>, &Vec<usize>, i32)>::new();
    while reduction_queue.len() > 0 {
        let queue_message = reduction_queue.pop_front().unwrap();
        if cache.contains(&queue_message) {
            continue;
        }
        cache.insert(queue_message.clone());
        let (processed_joltage, button_to_press, press_count) = queue_message;
        if processed_joltage[min_press_joltage.0] == target_joltage[min_press_joltage.0] {
            buttons
                .iter()
                .filter(|btn| btn.iter().filter(|idx| **idx != min_press_joltage.0).all(|&idx| processed_joltage[idx] <= target_joltage[idx]))
                .map(|btn| (processed_joltage.clone(), btn, press_count))
                .filter(|element| !cache.contains(element))
